...
function throttle(func, timeDelay) {
    ...
    return function (...funcArgs) {
        lastCall = Date.now();
        let timeDiff = lastCall - lastCallTime;

        if (timeDiff >= timeDelay) {
            executedBlock(funcArgs);
        }
        else {
            if (timer !== null) {
                clearTimeout(timer);
            ...
            timer = setTimeout(() => executedBlock(funcArgs), timeDelay - timeDiff);
        }
    }
}
...
function throttleAlt(func, timeDelay) {
    let bindingObj;
    let bindingArgs = null;
    let allowExecute = true;

    return wrapper;

    function wrapper(...funcArgs) {
        if (!allowExecute) {
            bindingObj = this;
            bindingArgs = funcArgs;
            return;
        }
        ...
        setTimeout(() => {
            ...
            if (bindingArgs) {
                wrapper.apply(bindingObj, bindingArgs);
                bindingArgs = null;
            }
        }, timeDelay);
    }
